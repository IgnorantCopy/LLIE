...
class DCENet(nn.Module):
    def __init__(self, in_channels: int = 3, base_channels: int = 32, n_iterations: int = 8):
        ...
        self.conv3 = ConvBlock(base_channels, base_channels)
        self.conv4 = ConvBlock(base_channels, base_channels)
        self.conv5 = ConvBlock(base_channels * 2, base_channels)
        self.conv6 = ConvBlock(base_channels * 2, base_channels)
        self.conv7 = ConvBlock(base_channels * 2, in_channels)
    ...
    def forward(self, x: torch.Tensor, return_a: bool = False):
        ...
        x_r = self.tanh(self.relu(self.conv7(torch.cat([x1, x6], 1))))
...
class ZeroDCEPlus(pl.LightningModule):
    def __init__(self, config, logger: "loguru.Logger"):
        ...
        self.lambda_tv = model_config.get('lambda_tv', 160.0)
